...
int _camera_set_evas_rendering(camera_h camera, bool enable)
{
	...
	if (enable) {
		...
		ret = mm_evas_renderer_retrieve_all_packets(pc->cb_info->evas_info, false);
...
int camera_start_preview(camera_h camera)
{
	...
	if (ret != CAMERA_ERROR_NONE) {
		...
		ret = _camera_set_evas_rendering(camera, true);
	}
...
int camera_stop_preview(camera_h camera)
{
	...
	if (CHECK_PREVIEW_CB(pc->cb_info, PREVIEW_CB_TYPE_EVAS)) {
		...
		if (current_state == CAMERA_STATE_PREVIEW) {
			ret = _camera_set_evas_rendering(camera, false);
			if (ret != CAMERA_ERROR_NONE) {
				return ret;
			}
		}
