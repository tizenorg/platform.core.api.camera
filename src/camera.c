...
int _camera_media_packet_create(camera_cb_info_s *cb_info, camera_stream_data_s *stream, int tbm_key, int num_buffer_key, tbm_bo bo, tbm_bo *buffer_bo, media_packet_h *packet)
{
	...
	case MUSE_CAMERA_EVENT_TYPE_MEDIA_PACKET_PREVIEW:
		{
			...
			if (cb_info->user_cb[MUSE_CAMERA_EVENT_TYPE_MEDIA_PACKET_PREVIEW]) {
				ret = _camera_media_packet_create(cb_info, stream, tbm_key, num_buffer_key, bo, buffer_bo, &pkt);
			...
			if (cb_info->user_cb[MUSE_CAMERA_EVENT_TYPE_PREVIEW] || ret != CAMERA_ERROR_NONE) {
				/* release imported bo */
				for (i = 0 ; i < num_buffer_key ; i++)
					_release_imported_bo(&buffer_bo[i]);
