...
int _camera_media_packet_create(camera_cb_info_s *cb_info, camera_stream_data_s *stream, int tbm_key, int num_buffer_key, tbm_bo bo, tbm_bo *buffer_bo, media_packet_h *packet)
{
	...
	if (cb_info == NULL || stream == NULL || bo == NULL || buffer_bo == NULL || packet == NULL) {
		LOGE("invalid parameter - cb_info:%p, stream:%p, bo:%p, buffer_bo:%p, packet:%p",
			cb_info, stream, bo, buffer_bo, packet);
		return CAMERA_ERROR_INVALID_PARAMETER;
	}
