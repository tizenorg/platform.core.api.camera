				...
				if (ret == CAMERA_ERROR_NONE) {
					...
					if (ret == CAMERA_ERROR_NONE) {
						...
						if (cb_info->evas_info)
							...
							LOGW("evas renderer is destroyed, skip this buffer...");
							media_packet_destroy(pkt);
						}
...
int camera_start_preview(camera_h camera)
{
	...
	if (CHECK_PREVIEW_CB(pc->cb_info, PREVIEW_CB_TYPE_EVAS)) {
		...
		if (pc->cb_info->evas_info == NULL)
			ret = mm_evas_renderer_create(&pc->cb_info->evas_info, (Evas_Object *)pc->display_handle);

		g_mutex_unlock(&pc->cb_info->evas_mutex);

		if (ret != MM_ERROR_NONE) {
			LOGE("failed to create mm evas renderer 0x%x", ret);
			return CAMERA_ERROR_INVALID_OPERATION;
		}
	}
...
int camera_stop_preview(camera_h camera)
{
	...
	if (CHECK_PREVIEW_CB(pc->cb_info, PREVIEW_CB_TYPE_EVAS)) {
		...
		/* destroy evas renderer */
		ret = mm_evas_renderer_destroy(&pc->cb_info->evas_info);

		g_mutex_unlock(&pc->cb_info->evas_mutex);

		if (ret != MM_ERROR_NONE) {
			LOGE("failed to destroy mm evas renderer 0x%x", ret);
			return CAMERA_ERROR_INVALID_OPERATION;
		}
	}
