...
void _camera_evas_surface_rendering_test(camera_cb_info_s *cb_info, camera_preview_buf_s *preview_buf)
{
	int i = 0;
	tbm_bo bo = NULL;
	...
	if (!_import_tbm_key(cb_info->bufmgr, preview_buf->tbm_key, &bo, &bo_handle)) {
		LOGE("failed to import key %d", preview_buf->tbm_key);

		((camera_preview_buf_cb)cb_info->user_cb[MUSE_CAMERA_EVENT_TYPE_PREVIEW2])(cb_info, preview_buf,
	...
	for (i = 0 ; i < preview_buf->num_buffer_key ; i++) {
		...
		if (!_import_tbm_key(cb_info->bufmgr, preview_buf->buffer_key[i], &buffer_bo[i], &buffer_bo_handle[i])) {
	...
	case MUSE_CAMERA_EVENT_TYPE_MEDIA_PACKET_PREVIEW:
		{
			...
			if (event == MUSE_CAMERA_EVENT_TYPE_PREVIEW) {
				...
				_camera_evas_surface_rendering_test(cb_info, preview_buf);
			#endif
			}
		}
		break;
